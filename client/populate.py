...
class ConfigPopulator(object):
    ...
    def prompt(self, variable, name, description=None, sanitize_func=None,
               checkdepends_func=None):
        if checkdepends_func is not None and not checkdepends_func(self):
            return
        if description:
            print(description)
        current_value = self.config.get(variable)
        if not self.sanitize_yesno(get_input(("%s (Current Value is: %r). " +
                                              "Edit? [yN] ")
                                             % (name, current_value))):
            print("")
            return
    ...
    def sanitize_location(self, location):
        ...
        if num_entries == 0:
            raise EnterAgainException('Weather not found. Please try ' +
                                      'another location.')
        else:
    ...
    def sanitize_sttengines(self, engine_id):
        try:
            ...
            try:
                idx = [name for name, desc
                       in self.AVAILABLE_STT_ENGINES].index(engine_id)
            except IndexError:
                EnterAgainException(("%r is neither a number nor a valid " +
                                     "name. Can't recognize STT engine.")
                                    % engine_id)
        else:
            idx -= 1
        try:
            ...
            EnterAgainException("%r is neither a number nor a valid name. " +
                                "Can't recognize STT engine." % engine_id)
        else:
            return name
    ...
    SETTINGS = [
        ...
        [['prefers_email'],
         ...
         sanitize_yesno,
         lambda x: (x.checkdepends_key('phonenumber') and
                    x.checkdepends_key('gmail_address'))],
        [['stt_engine'],
         'Speech-To-Text (STT) engine',
         '\n'.join(['If you would like to choose a specific STT ' +
                    ' engine, please specify which.'] +
                   ["  %d. %s" % (i, info[1]) for i, info in
                    enumerate(AVAILABLE_STT_ENGINES, start=1)]),
         sanitize_sttengines,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file", help="profile file to edit", nargs="?",
        default=jasperconf.DEFAULT_CONFIG_TYPE.STANDARD_CONFIG_FILE)
    parser.add_argument(
        "--new",
        help=("Start a new file from default file (default if file does not " +
              "exist)"),
        action="store_true")
    args = parser.parse_args()

    if args.new or not os.path.exists(args.file):
        suffix = '.%s' % jasperconf.DEFAULT_CONFIG_TYPE.FILE_EXT
